...
{
  font_location = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_ROBOTO_10));

  location_layer = text_layer_create(frame);
  text_layer_set_text_color(location_layer, GColorWhite);
  text_layer_set_background_color(location_layer, GColorClear);
  text_layer_set_font(location_layer, font_location);
...
void location_layer_update(WeatherData *weather_data) 
{
  ...
  text_layer_set_text(location_layer, "21:45 Amsterdam");
...
void location_layer_destroy() 
{
  ...
  fonts_unload_custom_font(font_location);
}
  ...
  if (weather_data->error) {
    ...
    */
    ...
    if (weather_data->hourly_updated != 0 && weather_data->hourly_enabled) {
      ...
      // Update Locale+time in upper left corner
      ...
      snprintf(locale_msg, sizeof(locale_msg), "%s, %s", last_update_text, weather_data->locale);
