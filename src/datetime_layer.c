...
static char date_text[] = "XXX 00 WXX";
...
void date_layer_update(struct tm *tick_time)
{
  ...
  char day_text[4];
  char weeknumber_text[3];
  strftime(day_text, sizeof(day_text), "%a", tick_time);
  strftime(weeknumber_text, sizeof(weeknumber_text), "%V", tick_time);
  snprintf(date_text, sizeof(date_text), "%s %i W%s", day_text, tick_time->tm_mday, weeknumber_text);
